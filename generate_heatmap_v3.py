#!/usr/bin/env python3
"""
GitHub Contribution Heatmap Generator - Versão 3 (Fogo Pulsante com Brilho)
...
COLORS = {
    ...
    8: "rgb(255,0,0)",      # 10-14 commits - Fogo Vermelho
    9: "#00EEFF",           # 15+ commits - Fogo Azul (Super Intenso)
}

def get_level(count):
    """Converte número de commits em nível de cor"""
    if count == 0: return 0
    if count >= 11: return 9  # Fogo Azul (Super Intenso)
...
def generate_svg_heatmap(contributions_data):
    ...
    height = 7 * total_cell + 110  # Espaço otimizado para legenda
    ...
    <style>
        ...
        /* Fogo Azul (Nível 9) - Super Intenso */
        .fire-blue {{ 
            rx: 2;
        ...
        @keyframes pulseFireBlue {{
            0% {{ 
                filter: 
                    drop-shadow(0 -6px 12px rgba(0, 150, 255, 1))
                    drop-shadow(0 -12px 24px rgba(0, 200, 255, 0.9))
                    drop-shadow(0 -20px 40px rgba(100, 220, 255, 0.7));
            }}
            50% {{ 
                filter: 
                    drop-shadow(0 -10px 20px rgba(0, 100, 255, 1))
                    drop-shadow(0 -20px 40px rgba(0, 200, 255, 1))
                    drop-shadow(0 -30px 60px rgba(150, 230, 255, 0.8));
            }}
            100% {{ 
                filter: 
                    drop-shadow(0 -6px 12px rgba(0, 150, 255, 1))
        ...
        /* Fagulhas Azuis (Fogo Azul) - Mais Rápidas */
        .spark-blue {{
            fill: #00ddff;
            opacity: 0;
            animation: sparkFlyBlue 1s ease-out infinite;
        }}
        ...
        @keyframes sparkFlyBlue {{
            ...
            100% {{
                opacity: 0;
                transform: translateY(var(--ty)) scale(0.1);
            }}
        }}
    </style>
    ...
    for week_idx, week in enumerate(weeks):
        ...
        for day_idx, day in enumerate(week["contributionDays"]):
            ...
            if level == 9:  # Fogo Azul (Super Intenso)
                ...
                # Adiciona 6 fagulhas azuis (mais rápidas e intensas)
                import random
                for i in range(6):  # 6 fagulhas para fogo azul
                    cx = x + cell_size / 2
                    cy = y + cell_size / 2
                    ty = random.randint(-45, -25)  # Mais alto que o vermelho
                    delay = random.uniform(0, 1.0)  # Delay mais curto
                    
                    spark = f'    <circle class="spark-blue" cx="{cx}" cy="{cy}" r="1.5" style="--ty: {ty}px; animation-delay: {delay}s;"/>\n'
                ...
                # Adiciona 4 fagulhas vermelhas
                import random
                for i in range(4):  # 4 fagulhas para fogo vermelho
                    cx = x + cell_size / 2
                    cy = y + cell_size / 2
                    ty = random.randint(-30, -18)  # Menos alto que o azul
                    delay = random.uniform(0, 1.5)
                    
                    spark = f'    <circle class="spark-red" cx="{cx}" cy="{cy}" r="1.5" style="--ty: {ty}px; animation-delay: {delay}s;"/>\n'
                    fire_cells.append(spark)
            else:
                class_name = "day"
                fill = color
    ...
    <defs>
        ...
        <linearGradient id="fireGradientBlue" x1="0%" y1="100%" x2="100%" y2="0%">
            <stop offset="0%" style="stop-color:#0066ff;stop-opacity:1">
                <animate attributeName="stop-color" values="#0066ff;#0044cc;#0066ff" dur="1.5s" repeatCount="indefinite"/>
            </stop>
            <stop offset="40%" style="stop-color:#0099ff;stop-opacity:1">
                <animate attributeName="stop-color" values="#0099ff;#00bbff;#0099ff" dur="1.5s" repeatCount="indefinite"/>
            </stop>
            <stop offset="70%" style="stop-color:#00ccff;stop-opacity:1">
                <animate attributeName="stop-color" values="#00ccff;#00eeff;#00ccff" dur="1.5s" repeatCount="indefinite"/>
            </stop>
            <stop offset="100%" style="stop-color:#00ffff;stop-opacity:1">
                <animate attributeName="stop-color" values="#00ffff;#99ffff;#00ffff" dur="1.5s" repeatCount="indefinite"/>
            </stop>
        </linearGradient>
    </defs>
...
def main():
    print(f"🔥 GitHub Heatmap Generator v3 - Fogo Pulsante")
    ...
    if not contributions or contributions["totalContributions"] == 0:
        ...
        print(f"   python generate_heatmap_v3.py")
    ...
    # Salva
    output_file = "github-heatmap-v3.svg"
    ...
    print(f"\n🎯 Adicione ao README:")
    print(f"   ![GitHub Heatmap](./github-heatmap-v3.svg)\n")
