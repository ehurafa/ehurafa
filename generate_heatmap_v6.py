#!/usr/bin/env python3
"""
GitHub Contribution Heatmap Generator - Versão 5 (Raio Elétrico)
...
COLORS = {
    ...
    9: "#FFFFFF",           # 12+ commits - Fogo Branco Incandescente!
}

def get_level(count):
    """Converte número de commits em nível de cor"""
    if count == 0: return 0
    if count >= 12: return 9  # Fogo Branco (Super Intenso)
...
def generate_svg_heatmap(contributions_data):
    ...
    <style>
        ...
        /* Fogo Branco (Nível 9) - Super Intenso com Alto Contraste */
        .fire-white {{ 
            rx: 2;
            animation: pulseFireWhite 1.5s ease-in-out infinite;
            filter: drop-shadow(0 0 15px rgba(255, 255, 255, 1));
        }}
        
        @keyframes pulseFireOrange {{
            0% {{
                filter: 
                drop-shadow(0 -6px 12px rgba(200, 50, 0, 1))
                drop-shadow(0 -12px 24px rgba(255, 255, 255, 1))
                drop-shadow(0 -20px 40px rgba(255, 160, 50, 0.9));
            }}
            50% {{
                filter: 
                drop-shadow(0 -10px 20px rgba(180, 30, 0, 1))
                drop-shadow(0 -20px 40px rgba(255, 255, 255, 1))
                drop-shadow(0 -30px 60px rgba(255, 200, 80, 1));
            }}
            100% {{
                filter: 
                drop-shadow(0 -6px 12px rgba(200, 50, 0, 1))
        ...
        /* Fagulhas Azuis (Fogo Branco) - Pequenas e Dispersas */
        .spark-white {{
            opacity: 0;
            animation: sparkFlyWhite 1.5s ease-out infinite;
        }}
        
        @keyframes sparkFlyWhite {{
            0% {{
                opacity: 0.9;
        ...
        @keyframes lightningStrike {{
            ...
            20% {{
                opacity: 1;
                stroke-width: 2.5;
                filter: drop-shadow(0 0 12px #00FFFF) drop-shadow(0 0 20px #FFFFFF);
            }}
            25% {{
                opacity: 0;
            }}
...
'''
    ...
    for week_idx, week in enumerate(weeks):
        ...
        for day_idx, day in enumerate(week["contributionDays"]):
            ...
            if level == 9:  # Fogo Branco (Super Intenso)
                class_name = "day fire-white"
                fill = "url(#fireGradientWhite)"
                rect_content = f'    <rect class="{class_name}" x="{x}" y="{y}" width="{cell_size}" height="{cell_size}" fill="{fill}"/>\n'
                fire_cells.append(rect_content)
                
                # Adiciona raio elétrico vertical atravessando a célula
                # Cria um caminho em zigue-zague de cima para baixo
                import random
                cx_center = x + cell_size / 2
                top_y = y - 5
                bottom_y = y + cell_size + 5
                
                # Pontos do raio em zigue-zague
                points = [
                    (cx_center, top_y),
                    (cx_center + random.uniform(-4, 4), y + cell_size * 0.2),
                    (cx_center + random.uniform(-6, 6), y + cell_size * 0.4),
                    (cx_center + random.uniform(-4, 4), y + cell_size * 0.6),
                    (cx_center + random.uniform(-5, 5), y + cell_size * 0.8),
                    (cx_center, bottom_y)
                ]
                ...
                lightning_delay = random.uniform(0, 2)
                lightning = f'    <path class="lightning" d="{path_d}" style="animation-delay: {lightning_delay}s;"/>\n'
                fire_cells.append(lightning)
    ...
    <defs>
        ...
        <!-- Gradiente Fogo Branco (Alto Contraste) -->
        <linearGradient id="fireGradientWhite" x1="0%" y1="100%" x2="100%" y2="0%">
            <stop offset="0%" style="stop-color:#ff0000;stop-opacity:1">
                <animate attributeName="stop-color" values="#ff0000;#cc0000;#ff0000" dur="1.5s" repeatCount="indefinite"/>
            </stop>
            <stop offset="30%" style="stop-color:#ff6600;stop-opacity:1">
                <animate attributeName="stop-color" values="#ff6600;#ff8800;#ff6600" dur="1.5s" repeatCount="indefinite"/>
            </stop>
            <stop offset="60%" style="stop-color:#ffdd00;stop-opacity:1">
                <animate attributeName="stop-color" values="#ffdd00;#ffff00;#ffdd00" dur="1.5s" repeatCount="indefinite"/>
            </stop>
            <stop offset="100%" style="stop-color:#ffffff;stop-opacity:1">
                <animate attributeName="stop-color" values="#ffffff;#ffffcc;#ffffff" dur="1.5s" repeatCount="indefinite"/>
            </stop>
        </linearGradient>
    </defs>
...
'''
    ...
    for level in range(10):  # Agora vai até 9 (10 níveis: 0-9)
        if level == 9:  # Fogo Branco
            svg += f'        <rect class="day fire-white" x="{legend_x + level * (cell_size + 2)}" width="{cell_size}" height="{cell_size}" fill="url(#fireGradientWhite)" rx="2"/>\n'
...
def main():
    print(f"⚡ GitHub Heatmap Generator v5 - Raio Elétrico")
    ...
    if not contributions or contributions["totalContributions"] == 0:
        ...
        print(f"   python generate_heatmap_v5.py")
    ...
    # Salva
    output_file = "github-heatmap-v5.svg"
    ...
    print(f"\n🎯 Adicione ao README:")
    print(f"   ![GitHub Heatmap](./github-heatmap-v5.svg)\n")
